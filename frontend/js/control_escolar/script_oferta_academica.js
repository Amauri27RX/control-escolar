document.addEventListener('DOMContentLoaded', function() {
    ...
    let listaDocentes = []; // Guardaremos la lista de docentes aquí
    ...
    async function cargarDatosIniciales() {
        try {
            const response = await fetch('/control-escolar/backend/control_escolar/api/api_oferta_academica.php?action=get_initial_data');
            ...
            result.data.programas.forEach(prog => filtroPrograma.add(new Option(prog.nombre_programa, prog.dgp)));
            
            listaDocentes = result.data.docentes; // Guardar lista de docentes
            
            // Cargar la tabla con los valores por defecto
            cargarOferta();
    ...
    async function cargarOferta() {
        ...
        if (!id_ciclo || !dgp) {
            tablaOfertaBody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">Seleccione un ciclo y un programa para continuar.</td></tr>';
            return;
        }

        tablaOfertaBody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">Cargando...</td></tr>';
        
        try {
            const response = await fetch(`/control-escolar/backend/control_escolar/api/api_oferta_academica.php?action=get_oferta&id_ciclo=${id_ciclo}&dgp=${dgp}`);
            const result = await response.json();
            if (!result.success) throw new Error(result.message);
    ...
    function renderTablaOferta(ofertaData) {
        ...
        if (ofertaData.length === 0) {
            tablaOfertaBody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">Este programa no tiene materias asignadas.</td></tr>';
            return;
        }
        ...
        ofertaData.forEach(materia => {
            ...
            // Crear el select de docentes para esta fila
    ...
    async function handleGuardarOferta(e) {
        ...
        try {
            const response = await fetch('/control-escolar/backend/control_escolar/api/api_oferta_academica.php?action=save_oferta', {
                method: 'POST',
